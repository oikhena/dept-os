...
import { TXT, SP, RAD, LS, LH, FONT, CLR, T } from "../../styles/tokens";
...
interface AgentsTabProps {
  ...
  accentColor: string;
  pulsePhase: number;
  onUpdateDept?: (dept: Department) => void;
}
...
export default function AgentsTab({ dept, accentColor, pulsePhase, onUpdateDept }: AgentsTabProps) {
  ...
  const handleSaveAgent = (agent: Agent) => {
    if (!onUpdateDept) return;
    const existing = agents.findIndex(a => a.id === agent.id);
    const updated = existing >= 0
      ? agents.map(a => a.id === agent.id ? agent : a)
      : [...agents, agent];
    onUpdateDept({ ...dept, agents: updated });
  };

  const handleDeleteAgent = (id: string) => {
    if (!onUpdateDept) return;
    onUpdateDept({ ...dept, agents: agents.filter(a => a.id !== id) });
    setSel(null);
    setConfirmDeleteId(null);
  };

  const handlePriorityChange = (agent: Agent, delta: number) => {
    if (!onUpdateDept) return;
    const newP = Math.max(1, Math.min(5, agent.priority + delta));
    if (newP === agent.priority) return;
    const updated = agents.map(a => a.id === agent.id ? { ...a, priority: newP } : a);
    onUpdateDept({ ...dept, agents: updated });
  };
  ...
  return (
    <div style={{ display:"flex", gap:0, minHeight:500 }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {/* Legend strip + Add Agent button */}
        <div style={{ display:"flex", gap:SP.lg, padding:`0 0 ${SP.sm}px 0`, flexWrap:"wrap", alignItems:"center" }}>
          {LANE_ORDER.map(type => {
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:SP.sm }}>
            <span style={{ fontSize:TXT.sm, color:T.textMuted }}>CLICK AGENT TO INSPECT</span>
            {onUpdateDept && (
              <Button variant="success" size="sm" onClick={() => setCreating(true)}>
                + Add Agent
              </Button>
            )}
          </div>
        ...
        {noAgents ? (
          <div style={{ background:T.bgSurface, border:`1px dashed ${T.borderDefault}`, borderRadius:RAD.lg, padding:40, textAlign:"center" }}>
            <div style={{ fontSize:32, marginBottom:SP.md, opacity:0.2 }}>🤖</div>
            <div style={{ fontSize:TXT.md, color:T.textMuted, marginBottom:SP.sm }}>No agent architecture yet</div>
            <div style={{ fontSize:TXT.md, color:T.textSecondary, lineHeight:LH.relaxed, marginBottom:SP.lg }}>
              Use the chat or generator to research this institution and produce a full agent map.
            </div>
            {onUpdateDept && (
              <Button variant="success" size="md" onClick={() => setCreating(true)}>
                + Create First Agent
              </Button>
            )}
          </div>
        ) : (
          <svg viewBox={`0 0 ${vbW} ${vbH}`} style={{ width:"100%", display:"block", background:T.bgPrimary, borderRadius:6, border:`1px solid ${T.borderDefault}` }}>
            <defs>
              {LANE_ORDER.map(type => (
              ...
              return (
                <g key={agent.id} onClick={() => setSel(isSel ? null : agent.id)} style={{ cursor:"pointer" }}>
                  ...
                  <circle cx={pos.x} cy={pos.y} r={r}
                    fill={isSel ? at.color+"22" : T.bgSurface}
                    stroke={at.color}
                  ...
                  <text x={pos.x} y={pos.y + 13} textAnchor="middle"
                    fill={isSel ? at.color : T.textMuted}
                  ...
                  <circle cx={pos.x + r - 0.5} cy={pos.y - r + 0.5} r={2.5}
                    fill={T.bgPrimary} stroke={at.color} strokeOpacity={0.5} strokeWidth={0.3} />
                  ...
                  {hasHil && (
                    <circle cx={pos.x - r + 0.5} cy={pos.y - r + 0.5} r={1.8}
                      fill={T.bgPrimary} stroke="#f59e0b" strokeOpacity={0.6 + 0.4 * Math.sin((pulsePhase||0)*0.06)} strokeWidth={0.3}>
                    </circle>
                  )}
                  {hasHil && (
        ...
        {!noAgents && (
          <div style={{ display:"flex", alignItems:"center", gap:SP.sm, marginTop:SP.sm, opacity:0.4 }}>
            <span style={{ fontSize:TXT.sm, color:T.textMuted }}>SENSOR / PHYSICAL WORLD</span>
            <div style={{ flex:1, height:1, background:"linear-gradient(to right, #34d399, #38bdf8, #a78bfa, #f59e0b)", opacity:0.4 }} />
            <span style={{ fontSize:TXT.sm, color:T.textMuted }}>ORCHESTRATION / SYNTHESIS</span>
          </div>
        )}
      </div>
      ...
      {selAgent && at && (() => {
        ...
        return (
          <div style={{ width:280, flexShrink:0, borderLeft:`1px solid ${T.borderDefault}`, padding:`${SP.lg}px ${SP.xl}px`, overflowY:"auto", background:T.bgSecondary }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:SP.lg }}>
              <div>
                ...
                <div style={{ fontSize:TXT.lg, color:T.textPrimary, fontWeight:700, lineHeight:LH.tight }}>{selAgent.name}</div>
                <div style={{ fontSize:TXT.sm, color:at.color, marginTop:3 }}>{at.label}</div>
              </div>
              <div style={{ display:"flex", gap:4 }}>
                {onUpdateDept && (
                  <>
                    <IconButton label="Edit agent" onClick={() => setEditing(selAgent)} size={28} variant="ghost">✎</IconButton>
                    <IconButton label="Delete agent" onClick={() => setConfirmDeleteId(selAgent.id)} size={28} variant="ghost">🗑</IconButton>
                  </>
                )}
                <IconButton label="Close panel" onClick={() => setSel(null)} size={28} variant="ghost">✕</IconButton>
              </div>
            </div>

            {/* Delete confirmation */}
            {confirmDeleteId === selAgent.id && (
              <div style={{
                background: `${CLR.danger}08`, border: `1px solid ${CLR.danger}30`,
                borderRadius: RAD.md, padding: SP.sm, marginBottom: SP.md,
                display: "flex", alignItems: "center", gap: SP.sm,
              }}>
                <span style={{ fontSize: TXT.sm, color: CLR.danger, flex: 1, fontFamily: FONT.sans }}>Delete this agent?</span>
                <Button variant="danger" size="sm" onClick={() => handleDeleteAgent(selAgent.id)}>Delete</Button>
            ...
            <div style={{ display:"flex", gap:SP.xs, flexWrap:"wrap", marginBottom:SP.lg, alignItems:"center" }}>
              {onUpdateDept && (
                <div style={{ display:"flex", gap:2, marginRight:4 }}>
                  <IconButton label="Increase priority" onClick={() => handlePriorityChange(selAgent, -1)} size={22} variant="ghost">▲</IconButton>
                  <IconButton label="Decrease priority" onClick={() => handlePriorityChange(selAgent, 1)} size={22} variant="ghost">▼</IconButton>
                </div>
              )}
              ...
              <span style={{ background:T.bgSurface, border:`1px solid ${T.borderDefault}`, color:T.textMuted, borderRadius:RAD.sm, padding:"2px 7px", fontSize:TXT.sm }}>{selAgent.complexity} complexity</span>
            ...
            {[{l:"TRIGGER",v:selAgent.trigger,c:"#60a5fa"},{l:"OUTPUT",v:selAgent.output,c:"#22c55e"},{l:"HUMAN IN LOOP",v:selAgent.humanInLoop,c:"#f59e0b"}].map(f => (
              <div key={f.l} style={{ background:T.bgPrimary, border:`1px solid ${f.c}20`, borderLeft:`2px solid ${f.c}`, borderRadius:RAD.sm, padding:`${SP.sm}px ${SP.sm}px`, marginBottom:SP.sm }}>
                <div style={{ fontSize:TXT.sm, color:f.c, letterSpacing:LS.wide, marginBottom:3 }}>{f.l}</div>
                <div style={{ fontSize:TXT.md, color:T.textSecondary, lineHeight:LH.normal }}>{f.v || "—"}</div>
              </div>
            ))}
            ...
            {selAgent.integrations && selAgent.integrations.length > 0 && (
              <div style={{ marginBottom:SP.sm }}>
                <div style={{ fontSize:TXT.sm, color:T.textMuted, letterSpacing:LS.wide, marginBottom:SP.xs }}>INTEGRATIONS</div>
                <div style={{ display:"flex", gap:SP.xs, flexWrap:"wrap" }}>
                  {selAgent.integrations.map((x: string, i: number) => (
                    <span key={i} style={{ background:T.bgSurface, border:`1px solid ${T.borderDefault}`, color:T.textSecondary, borderRadius:RAD.sm, padding:"2px 7px", fontSize:TXT.sm }}>{x}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {(kw.length > 0 || sens.length > 0) && (
              <div>
                <div style={{ fontSize:TXT.sm, color:T.textMuted, letterSpacing:LS.wide, marginBottom:SP.sm }}>HANDLES</div>
                <div style={{ display:"flex", flexDirection:"column", gap:SP.xs }}>
                  ...
                  {sens.map((s: any) => (
                    <div key={s.id} style={{ background:"#a78bfa10", border:"1px solid #a78bfa25", borderRadius:RAD.sm, padding:`${SP.xs}px ${SP.sm}px` }}>
                      ...
                      <div style={{ fontSize:TXT.sm, color:T.textSecondary, marginTop:1 }}>{s.sense} sensor</div>
                    </div>
                  ))}
                </div>
              </div>
